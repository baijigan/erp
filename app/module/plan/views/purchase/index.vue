		...
		<scroll-view class="scroll-list msg-list-item" scroll-y="true">
			<view v-for="(item, index) in detailTableData" :key="index">
				<view style="padding-left: 10px;padding-right: 10px;padding-top: 10px; background-color: #ffffff;">
					<view>{{item.inv_code}} {{item.inv_name}} {{item.invAttribute}}</view>
					<view style="color: #c3c3c3;">
						<table>
							<tr style="border-spacing: 0px;">
								<td width="25%">采购数量：</td><td width="20%">{{item.quantity}}</td>
								<td width="25%">已采数量：</td><td width="20%">{{item.wi_quantity}}</td>
							</tr>
							<tr>
								<td width="25%">需求日期：</td><td width="20%" colspan="3">{{inMaterTime(item.need_date)}}</td>
							</tr>							
						</table>
					</view>
      ...
      return new Promise((resolve) => {
        dictMatching( "mp_need_type").then((res) => {
          if (res.code == 200) {
            ...
            if(this.$route.query.needType!=''){
              this.queryDetail.needType = this.$route.query.needType;
            }else
            {
              this.queryDetail.needType = res.data[0].dictValue;
            }
            resolve();
          }
        });
        ...
        listDetail(this.queryDetail).then((res) => {
          if (res.code == 200) {
            ...
            if (res.rows.length > 0) {
              ...
              let last = list[0].mp_code,
                tag = 0;
              list.forEach((item, i) => {
                if (list[i].mp_code !== last) {
                  if (tag == 1) {
                    tag = 0;
                  } else if (tag == 0) {
                    tag = 1;
